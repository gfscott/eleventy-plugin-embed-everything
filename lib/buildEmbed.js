...
function liteEmbed(id, options, index) {
  ...
  if ( index === 0 ) {
    if ( liteOpt.css.inline ) {
      out += liteOpt.css.enabled ? `<style>${inlineCss}</style>\n` : '';
    } else {
      out += liteOpt.css.enabled ? `<link rel="stylesheet" href="${liteOpt.css.path}">\n` : '';
    }
    if ( liteOpt.js.inline ) {
      out += liteOpt.js.enabled ? `<script>${inlineJs}</script>\n` : '';
    } else {
      out += liteOpt.js.enabled ? `<script defer="defer" src="${liteOpt.js.path}"></script>\n` : '';
    }
  }
