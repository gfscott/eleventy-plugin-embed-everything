...
validStrings.forEach(obj => {
    ...
    test(`URL string returns expected media type for ${obj.type}`, t => {
    ...
    test(`URL string returns expected media ID for ${obj.type}`, t => {
        let str = `<p>https://open.${obj.str}</p>`;
    ...
    test(`URL string returns expected media object for ${obj.type}`, t => {
        let str = `<p>https://open.${obj.str}</p>`;
 ...
 */
test(`Playlist and User Playlist types in validStrings.js have identical IDs`, t => {

    let filteredArray = validStrings
        .filter(obj => obj.slug === 'playlist' || obj.slug === 'userPlaylist' );
